...
seedItems := ["Carrot", "Strawberry", "Blueberry", "Orange Tulip", "Tomato", "Corn", "Daffodil", "Watermelon", "Pumpkin", "Apple", "Bamboo", "Coconut", "Cactus", "Dragon Fruit", "Mango", "Grape", "Mushroom", "Pepper", "Cacao", "Beanstalk", "EmberLily", "Sugar Apple", "Burning Bud", "Giant Pinecone", "Elder Strawberry"]

gearItems := ["Watering Can", "Trowel", "Recall Wrench", "Basic Sprinkler", "Advanced Sprinkler", "Medium Toy", "Medium Treat", "Godly Sprinkler", "Magnifying Glass", "Tanning Mirror", "Master Sprinkler", "Cleaning Spray", "Favorite Tool", "Harvest Tool", "Friendship Pot", "Levelup Lollipop"]
...
EggShopPath:
    ...
    Sleep, 100  ; Kept original
    uiUniversal("11110")
    Sleep, 100  ; Kept original
    hotbarController(1, 0, "2")
    sleepAmount(100, 1000)  ; Kept original
    SafeClickRelative(midX, midY)  ; Kept original mouse click
    SendDiscordMessage(webhookURL, "**[Egg Cycle]**")
    Sleep, 800  ; Kept original

    ; Navigate to the egg NPC - kept original timing
    Send, {w Down}
    Sleep, 600  ; Kept original movement timing
    Send {w Up}
    sleepAmount(500, 1000)  ; Kept original
    Send {e}
    Sleep, 1200  ; Kept original for dialog appearance
    
    ; Zoom in - kept original timing
    Loop, 5 {
        Send, {WheelUp}
        Sleep, 30  ; Kept original
    }
    
    Sleep, 1000  ; Kept original delay before clicking
    
    ; Click on #1 option - kept original timing
    SafeClickRelative(0.92, 0.03)  ; Kept original mouse click
    
    Sleep, 1000  ; Kept original delay after clicking
    
    ; Zoom back out - kept original timing
    Loop, 5 {
        Send, {WheelDown}
        Sleep, 30  ; Kept original
    }
    
    ; Allow time for the egg shop GUI to appear - kept original
    sleepAmount(3000, 4000)
    
    ; Check if shop opened successfully
    eggsCompleted := 0
    Loop, 5 {
        if (simpleDetect(0x00CCFF, 10, 0.54, 0.20, 0.65, 0.325)) {
            ...
            ; Start UI navigation - kept original
            sendKeybind(SavedKeybind)
            Sleep, 500  ; Kept original
            
            ; Press S twice to get to first egg - sped up
            Send, {s}
            Sleep, 130  ; Faster
            Send, {s}
            Sleep, 170  ; Faster
            
            ; Loop through all eggs - UI navigation sped up
            Loop, % eggItems.Length() {
                currentItem := eggItems[A_Index]
                isSelected := 0
                
                ; Check if this egg is selected for purchase
                for i, selectedEgg in selectedEggItems {
                    if (currentItem = selectedEgg) {
                        isSelected := 1
                        break
                    }
                }
                
                ; Log which egg we're checking
                ToolTip, % "Checking: " . currentItem
                SetTimer, HideTooltip, -1000
                
                if (isSelected) {
                    ; Try to buy this egg regardless of stock status
                    SendDiscordMessage(webhookURL, "Attempting to buy " . currentItem)
                    
                    ; Select the egg - UI navigation sped up
                    Send, {Enter}
                    Sleep, 170  ; Faster
                    
                    ; Navigate to buy button - UI navigation sped up
                    Send, {s}
                    Sleep, 130  ; Faster
                    Send, {s}
                    Sleep, 170  ; Faster
                    
                    ; Spam click the buy button 3 times
                    Loop, 3 {
                        Send, {Enter}
                        Sleep, 270  ; Faster but still enough time for purchase
                    }
                    
                    SendDiscordMessage(webhookURL, "Attempted to buy " . currentItem)
                    
                    ; Press S once to go to next egg option - UI navigation sped up
                    Send, {s}
                    Sleep, 170  ; Faster
                }
                else {
                    ; Skip non-selected eggs - UI navigation sped up
                    Send, {s}
                    Sleep, 130  ; Faster
                    Send, {s}
                    Sleep, 170  ; Faster
                ...
                ; Additional delay between eggs
                Sleep, 230  ; Faster
            }
            
            ; Proper exit sequence for egg shop
            SendDiscordMessage(webhookURL, "Exiting egg shop...")
            
            ; First disable UI navigation - kept original timing
            sendKeybind(SavedKeybind)
            Sleep, 500  ; Kept original
            
            ; Re-enable UI navigation - kept original timing
            sendKeybind(SavedKeybind)
            Sleep, 500  ; Kept original
            
            ; Press keys in specific order to exit - sped up
            Send, {w}
            Sleep, 150  ; Faster
            Send, {w}
            Sleep, 150  ; Faster
            ...
            Sleep, 150  ; Faster
            Send, {Enter}
            Sleep, 500  ; Kept mostly original for reliability
            
            ; Exit UI navigation - kept original timing
            sendKeybind(SavedKeybind)
            Sleep, 500  ; Kept original
        ...
        Sleep, 2000  ; Kept original
    }
    
    closeRobuxPrompt()
    sleepAmount(1250, 2500)  ; Kept original
